...
//        return $this;
    ...
    private function processDuplicateValue() {
        ...
        if (array_count_values($minValueArr)[$max] === 1) {
            ...
            if ($this->row - count($this->doNotThinkRow) === 2 && $this->column - count($this->doNotThinkColumn) === 1) {
                echo '<h4>2x2</h4>';

                $sumDemand = array_sum($this->demand);

                // supply > demand ทั้ง 2 ค่าเลยหรือไม่
                $bool = true; $position = null;
                foreach ($this->supply as $item => $value) {
                    $bool &= $value > $sumDemand;
                    if ($value > $sumDemand) {
                        $position = $item;
                    }
                }
                ...
                if ($bool) {
                    ...
                    foreach ($this->positionMinZero as $item => $value) {
                        if ($value['row'] === $position) {
                            $this->positionMinZero = $this->positionMinZero[$item];
                        }
                    }
                }

                // TODO if $this->row - count($this->doNotThinkRow) === 1 && $this->column - count($this->doNotThinkColumn) === 2
            } else {
